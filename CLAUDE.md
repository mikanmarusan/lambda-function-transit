...
- A CI guard must be proven to fail, not just to pass. Before shipping one, run it against the violation it targets *and* against every way it can pass vacuously: `git diff` ignores untracked files, `grep` exits 2 (indistinguishable from "no match" inside `if`) on a missing file or a bad pattern, and a hard-coded path list silently stops covering files added later. Assert the inputs exist, derive path lists from `git ls-files`, and branch on the exit code explicitly so the gate fails closed.
