...
Detect and fix npm audit vulnerabilities, matching the CI security check (`npm audit --audit-level=high`).
...
`src/package.json` has no dependencies and is not audited.

## Execution

### Phase 1: Audit

1. Run `npm audit --audit-level=high` at root
...
### Fixes Applied
| Package | From | To | Method | Directory |
|---------|------|----|--------|-----------|
...
- Match CI: `--audit-level=high`
- Never use `npm audit fix --force`
- Never skip test verification after fixes
- Both `package.json` and `package-lock.json` changes must be committed
